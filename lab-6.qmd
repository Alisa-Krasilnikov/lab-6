...
remove_outliers <- function(data, ..., sd_thresh = 3) {
  ...
  non_numeric <- cols[!sapply(dt[, ..cols], is.numeric)]
  ...
  for (col in numeric_cols) {
    z <- as.vector(scale(dt[[col]])) #Get z-score
    mask <- mask & abs(z) <= sd_thresh #Filter out outliers
  }
  
  #Apply mask and return filtered dataset
  return(dt[mask, , drop = FALSE])
}
```
...
impute_missing_dt <- function(data, ..., impute_fun = mean) { #I don't remember why I named this impute_missing_dt but I'm worried it's going to cause errors if I change it
  #Capture column names
  cols <- as.character(substitute(list(...)))[-1]
...
df_arg_combos <- crossing(
  ...
  mod = list(
    ...
    price ~ carat + cut + clarity + color #Adjusting for cut, clarity, and color
  )
)
```
...
results <- pmap(df_arg_combos, function(impute, remove_outliers, mod) {
  # Call the fit_model function for each combination of arguments
  fit_model(diamonds, impute, remove_outliers, mod, price, carat)
})
